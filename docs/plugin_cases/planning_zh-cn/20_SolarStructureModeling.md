...
- **KDS 41 12:00:2022 风荷载完全自动计算**：勾选“自动生成风荷载”后，只输入基本风速（Vs）与地面
  粗糙度类别，即反映模块形状（倾角 12.0°、纯架率等），自动算出4个工况（WL1 β=0° A/迎风面、
  ...
  - 用**“计算书输出”按钮**可展开查看全部计算依据，实际显示的计算步骤为：
    ...
    5. 纯架结构荷载 — 柱设计速度压（q_h × G_pa）、柱截面宽度 d、线荷载 W（=P_F × d）。
...
- 自动生成风荷载：输入基本风速（Vs，例 30m/s）、地面粗糙度类别（例 C — 一般地区）、勾选纯架
...
- ⚠️ 用于生成“面荷载”（Floor Load）的确切端点（是 `/db/STLD` 系列的 Floor Load Type 还是另有专门
  端点）未在画面中显示，属推测。
- KDS 41 12:00:2022 风荷载计算本身（净压力系数 C_N、设计压力 P_W/P_L、纯架结构荷载的计算）并非
