...
输入桩参数（桩类型、上段·下段桩、加固）并布置桩后，再输入地层信息，即可查看各桩的特征值、轴向弹簧常数（Kv）与垂直轴向刚度（K1~K4），并可经 Excel 计算书查看各方向（Global X、Y）的柔性弹簧矩阵取值。最终将各方向矩阵组合成 6x6 General Spring 完成计算，并以 General Spring 的形式导入 midas Civil。
